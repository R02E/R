...
plot(iris$Sepal.Length, iris$Sepal.Width,
     ...
     col= as.numeric(factor(iris$Species)), family="AppleGothic") #꽃받침의 길이와 너비 간 산포도 출력. 종을 요인으로 인식함.
#2단계: 데이터 표준화
...
plot(data.scaled[,1], data.scaled[,2],
     xlab = "꽃받침 길이", ylab = "꽃받침 너비",
     pch = 21, col = kc$cluster, family="AppleGothic") #꽃받침의 길이와 너비의 산포도. 기호:모양(21), 색(군집번호)
legend("topright", legend = levels(iris$Species),
       pch = 21, col = kc$cluster,
       xpd = TRUE, inset = c(-0.4, 0)) #범례의 위치
points(kc$centers,
...
twss<-NULL
for(i in 1:15){kc<-kmeans(data.scaled, centers = i)
